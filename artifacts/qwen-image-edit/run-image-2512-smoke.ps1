# Smoke Image 2512 generate via SEA
param(
    ...
    [string]$Prompt = "Cinematic close-up of a futuristic CPU on a high-end motherboard, intense electric sparks flowing between circuits, ultra-detailed microchips, dramatic cyberpunk lighting, high contrast, metallic textures with reflections, shallow depth of field, sci-fi technology photography, sharp focus, no text.",
    [string]$OutputName = "image-2512-smoke.png",
...
$caps = curl.exe --fail --silent --show-error "$VideoHost/video/v1/capabilities" | ConvertFrom-Json
if ($caps.image_generate_ready -ne $true) { throw "image_generate_ready is false: $($caps | ConvertTo-Json -Compress)" }
...
from guideants_video_client import submit_image_generate
import json
print(json.dumps(submit_image_generate(
    '$escapedPrompt',
    '$OutputName',
    parameters={'steps': 4, 'cfg': 1.0, 'seed': 695101294492382},
), separators=(',', ':')))
"@
$sw = [System.Diagnostics.Stopwatch]::StartNew()
$submit = Invoke-SandboxExecute "gen submit" $submitPayload (Join-Path $ArtifactDir "gen-submit.json")
$jobId = Get-ResponseProperty $submit 'jobId'
Write-Host "gen job $jobId"
...
do {
    ...
    $status = Invoke-SandboxExecute "gen status" $statusPayload (Join-Path $ArtifactDir "gen-status.json")
    ...
    Write-Host "[gen] $message"
    if ($state -eq "completed") { break }
    if ($state -in @("failed", "cancelled")) { throw "gen ended in state '$state': $(Get-ResponseProperty $status 'error')" }
    if ((Get-Date) -ge $deadline) { throw "Timed out waiting for gen job $jobId" }
...
Invoke-SandboxExecute "gen materialize" $materializePayload (Join-Path $ArtifactDir "gen-materialize.json") | Out-Null
$sw.Stop()
$elapsed = [math]::Round($sw.Elapsed.TotalSeconds, 1)
Copy-Item -LiteralPath (Join-Path $OutputDir $OutputName) -Destination (Join-Path $ArtifactDir $OutputName) -Force
Write-Host "GEN_OK elapsed=$elapsed path=$(Join-Path $ArtifactDir $OutputName)"
