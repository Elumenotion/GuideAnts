...
public sealed class LocalAiDesiredStateBuilderTests
{
    ...
    [TestMethod]
    public async Task BuildPlanJsonAsync_NeverContainsLlamaSection()
    {
        // Contract: the lifecycle plan carries ONLY auxiliary services. Llama
        // actuation is API-direct (IGlobalDefaultLlamaReconciler on default change,
        // NotebookModelRuntimeService on use), so the plan must never contain a
        // llama section - not even a disabled one.
        ...
        var dbOptions = new DbContextOptionsBuilder<ApplicationDbContext>()
            .UseInMemoryDatabase($"no-llama-plan-{Guid.NewGuid():N}")
            .Options;
        ...
        db.Models.Add(new Model
        {
            ...
            RuntimeConfigJson = $$"""{"routerModelId":"{{routerAlias}}"}""",
        });
        ...
        planJson.ToLowerInvariant().Should().NotContain("llama");
    }
