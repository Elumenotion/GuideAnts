...
- A new hash formula must still reuse an already-built image for those same inputs (previous path-based tag, `org.guideants.deps-input-hash` label, or the unlabeled `guideants-ai-deps:<backend>-cache` image as last resort). Missing `index.json` under `.buildx-cache-*` must not be passed as `--cache-from`.
