...
instead of loopback adapter ports. The gateway is a transparent reverse proxy:
  ...
  {BASE}/v1/talking-head/jobs
  {BASE}/v1/talking-head/jobs/{id}
  {BASE}/v1/talking-head/jobs/{id}/cancel
  {BASE}/v1/talking-head/jobs/{id}/result
  {BASE}/files

Auth: X-Talking-Head-Skill-Token. Stdlib-only.
"""
...
def skill_base_url() -> str | None:
    raw = (os.environ.get("TALKING_HEAD_SKILL_BASE_URL") or "").strip().rstrip("/")
    return raw or None


def skill_token() -> str | None:
    raw = (os.environ.get("TALKING_HEAD_SKILL_TOKEN") or "").strip()
    return raw or None
...
def gateway_env_missing_blocker() -> str:
    return (
        "skillGateway: TALKING_HEAD_SKILL_BASE_URL is not set — "
...
def require_gateway() -> None:
    if not using_skill_gateway():
        sys.stderr.write(
            f"TALKING_HEAD_SKILL_BASE_URL is not set. {gateway_env_missing_hint()}\n"
        )
        sys.exit(1)
...
def gateway_request(
    ...
    *,
    ...
    base = skill_base_url()
    if not base:
        raise RuntimeError("TALKING_HEAD_SKILL_BASE_URL is not set")
...
def gateway_request_multipart(
    ...
    *,
    ...
    if not base:
        raise RuntimeError("TALKING_HEAD_SKILL_BASE_URL is not set")
    boundary = "----guideantsTalkingHeadSkillBoundary7MA4YWxkTrZu0gW"
    body = bytearray()
...
def probe_gateway(timeout: float = 5.0) -> dict[str, Any]:
    ...
    if not skill_token():
        return {"configured": True, "open": False, "error": "TALKING_HEAD_SKILL_TOKEN missing"}
    try:
