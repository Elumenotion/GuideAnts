...
public sealed class ExtractAssistantFileMarkdownHandlerTests
{
    ...
    public async Task HandleAsync_Returns_false_when_shadow_missing()
    {
        ...
        var result = await handler.HandleAsync(new ExtractAssistantFileMarkdownJob(Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task HandleAsync_Returns_true_when_shadow_already_completed()
    {
        ...
        var result = await handler.HandleAsync(new ExtractAssistantFileMarkdownJob(assistantFileId), CancellationToken.None);
