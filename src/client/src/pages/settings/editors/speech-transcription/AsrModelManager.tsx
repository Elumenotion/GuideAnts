...
export function AsrModelManager({ enabled }: AsrModelManagerProps) {
  const [phase, setPhase] = useState<LocalCapabilityPhase>('loading');
  const [list, setList] = useState<AsrListPayload | undefined>(undefined);
  const [readiness, setReadiness] = useState<AsrReadiness | undefined>(undefined);
  ...
  const refresh = async (): Promise<void> => {
    ...
    setList(listOutcome.payload as AsrListPayload);
    if (readyOutcome.kind === 'available') {
      setReadiness(readyOutcome.payload as AsrReadiness);
    } else {
      setReadiness(undefined);
    }
    setPhase('available');
  ...
  return (
    <>
      ...
      >
        ...
        {list?.modelDir ? (
          <p className="text-xs text-gray-500">
            Model directory on the ASR container: <span className="font-mono">{list.modelDir}</span>
          </p>
        ) : null}
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <tbody>
              ...
              {items.map((m) => {
                ...
                const disableLoad = engineBusy !== null || !!m.active;
                const disableRemove = engineBusy !== null || !!m.active;
                return (
                  <tr key={m.modelRef} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-2 text-right">
                      <div
                        ...
                        role="group"
                        aria-label={`Actions for ASR model ${m.modelRef}`}
                      >
                        <IconActionButton
                          label="Load"
                          tone="success"
        ...
        <div className="flex justify-end">
          <TextActionButton
            ...
            title={engineBusy !== null ? 'Wait for the current engine operation to finish.' : 'Add a new ASR model from Hugging Face.'}
          >
            Add model
          </TextActionButton>
        </div>
      ...
      <ConfirmationDialog
        ...
        title="Remove ASR model"
        message={
          pendingRemove
            ? `Remove local ASR model "${pendingRemove}" from the container's model directory? This cannot be undone.`
            : ''
        }
        confirmText="Remove"
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <span className="font-semibold">ASR engine:</span>
