# AC3-only warm run (container already up; no restart)
param(
...
$submit = Invoke-SandboxExecute "ac3 submit" $submitPayload (Join-Path $ArtifactDir "ac3-submit.json")
$jobId = Get-ResponseProperty $submit 'jobId'
Write-Host "ac3 job $jobId"
...
do {
    ...
    $status = Invoke-SandboxExecute "ac3 status" $statusPayload (Join-Path $ArtifactDir "ac3-status.json")
    ...
    Write-Host "[ac3] $message"
    if ($state -eq "completed") { break }
    if ($state -in @("failed", "cancelled")) { throw "ac3 ended in state '$state': $(Get-ResponseProperty $status 'error')" }
    if ((Get-Date) -ge $deadline) { throw "Timed out waiting for ac3 job $jobId" }
    Start-Sleep -Seconds $PollSeconds
} while ($true)

$materializePayload = $common.Clone()
$materializePayload.script = "from guideants_video_client import materialize_image_result`nimport json`nprint(json.dumps(materialize_image_result('$jobId', '$WarmOutputName'), separators=(',', ':')))"
Invoke-SandboxExecute "ac3 materialize" $materializePayload (Join-Path $ArtifactDir "ac3-materialize.json") | Out-Null
$sw.Stop()
$elapsed = [math]::Round($sw.Elapsed.TotalSeconds, 1)
Copy-Item -LiteralPath (Join-Path $OutputDir $WarmOutputName) -Destination (Join-Path $ArtifactDir $WarmOutputName) -Force
Write-Host "AC3 warm completed in ${elapsed}s (job $jobId)"
if ($elapsed -gt 600) { throw "AC3 failed: warm job took ${elapsed}s (>600s)" }

# Reload evidence: Started time of container vs this job window
docker inspect compose-comfyui-video-1 --format "container_started={{.State.StartedAt}} status={{.State.Status}}"
Write-Host "AC3_OK elapsed=$elapsed"
