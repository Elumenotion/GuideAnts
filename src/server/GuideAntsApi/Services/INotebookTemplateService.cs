...
public interface INotebookTemplateService
{
    ...
    Task<IEnumerable<GuideAntsApi.Models.NotebookTemplateSummaryDto>> GetTemplateSummariesAsync(Guid? userId = null);

    /// <summary>
    /// Retrieve all notebook templates, enriched with data from their manifests.
    /// </summary>
    /// <param name="userId">Optional user ID to prioritize user-specific templates.</param>
    Task<IEnumerable<GuideAntsApi.Models.NotebookTemplateDto>> GetTemplatesAsync(Guid? userId = null);
    ...
    Task<GuideAntsApi.Models.NotebookTemplateDto?> GetTemplateByIdAsync(Guid templateId, Guid? userId = null);
    ...
    Task<IReadOnlyList<GuideAntsApi.Models.AssistantDefinitionDto>> GetAssistantsAsync(Guid templateId, Guid? userId = null);
