---
name: talking-head
description: "Make a talking-head MP4 on Max: avatar + audio + plate → 416×256 LongCat-Video-Avatar-1.5, CorridorKey @ 416×234, BasicVSR++ FG, 1280×720. Submit with video_tool.py, then poll status on later sandbox calls."
metadata:
  guideants:
    enabled: true
    display_order: 50
...
CLI defaults (do not add flags): `width=416 height=256 steps=8 cfg=1 fps=25 seed=-1`.
