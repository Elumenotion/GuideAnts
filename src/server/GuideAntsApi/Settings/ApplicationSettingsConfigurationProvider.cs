...

{
    ...
    private const string ChatDefaultsSectionName = "ChatDefaults";
...

    {
        ...
        if (definition is not null
            ...
            && string.Equals(sectionName, ChatDefaultsSectionName, StringComparison.Ordinal))

        {
...

            {
                ...
                FlattenChatDefaultsProperty(payload, property);
    ...
    /// <summary>
    /// ChatDefaults null/empty values must shadow lower-priority configuration sources
    /// (appsettings and env) so cleared temperature/top_p do not resurrect after reload.
    /// Other registry sections keep legacy behavior: absent keys do not override env.
    /// </summary>
    private void FlattenChatDefaultsProperty(JsonObject payload, SettingsPropertyDefinition property)

    {

        payload.TryGetPropertyValue(property.Name, out var node);

        if (node is JsonObject nestedObject)

        {

            FlattenNested($"{ChatDefaultsSectionName}:{property.Name}", nestedObject);
...

        {
            ...
            Data[property.CanonicalKey] = string.Empty;
        ...
        Data[property.CanonicalKey] = string.IsNullOrWhiteSpace(value) ? string.Empty : value;
