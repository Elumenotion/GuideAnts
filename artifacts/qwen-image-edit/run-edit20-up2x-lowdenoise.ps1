# 20-step edit on ffmpeg 2x source, denoise 0.45, megapixels 2.3
...
$OutName = "diagram-styled-20step-up2x-d045.png"
...
$parsed = $caps.Body | ConvertFrom-Json
if ($parsed.image_edit_20_ready -ne $true) { throw "image_edit_20_ready false: $($caps.Body)" }

$paramsPath = Join-Path $ArtifactDir "edit20-up2x-d045-params.json"
[IO.File]::WriteAllText($paramsPath, '{"steps":20,"cfg":4.0,"seed":42,"denoise":0.45,"shift":3.1,"megapixels":2.3}')
...
$submit = Get-HttpJson "$VideoHost/video/v1/image/jobs" @(
  ...
  "-F", "workflow_version=qwen-image-edit-20-v1",
...
Write-Host "submitted job=$jobId workflow=qwen-image-edit-20-v1 denoise=0.45 megapixels=2.3"
...
do {
  ...
  Write-Host ("[edit20-up2x-d045] {0}" -f $msg)
  if ($state -eq "completed") { break }
  if ($state -in @("failed", "cancelled")) { throw "ended $state : $($job.error)" }
  if ((Get-Date) -ge $deadline) { throw "timeout job $jobId after 1800s" }
