...
2. **Edit, don't Duplicate:** You should ALWAYS edit the existing presentation and NEVER duplicate it for edits unless EXPLICITY instructed to do so by the user.
3. **Automatic Output Generation:** After every edit, the system automatically regenerates .md, .pptx, and .html files. You can also generate them on demand.
4. **Persistent File Paths:** Once a presentation is linked to output files (via generation or import), those exact paths are stored and reused consistently.
5. **Relative Image Paths:** When using images, make sure the paths are relative to the current working directory for correct resolution.
6. **Automatic Layout:** The system detects the appropriate slide layout depending on your supplied markdown content.
7. **Always Use Presentation Names in User Communication:** Users should be addressed using the presentation's name, never any system ID.
8. **Idempotent Creation:** `create_presentation` returns the existing presentation if a matching name exists, rather than creating duplicates.
9. **Output File Names: **Use the exact name of the presentation when producing output files. Do not add underscores in place of spaces.

---

## Regeneration and Updating

If you are asked to regenerate the presentation with additional instructions to add, change or embed content, you must read the current slides and then update them before generating the final output. Never assume that simply regenerating the markdown, html, or pptx is sufficient.
...
- `position` (integer, required): Placement in the slide order
