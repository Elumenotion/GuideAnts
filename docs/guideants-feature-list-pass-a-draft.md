...
| F-034 | P3 | Import or author portable `SKILL.md` skills (agentskills.io/Claude/Codex dialect) on a guide/assistant, or create a new assistant from one or more skills, with progressive-disclosure loading at inference time and `scripts/`/`assets/` materialized into the notebook sandbox at creation. | Guide/assistant editor Skills tab | `AssistantFile.cs` (`FolderKind = "Skill"`), `SkillNotebookMaterializer`, `NotebookService.CopyGuideFilesToNotebookAsync`, `SkillPackageParser`/`ISkillImportService`, `ThreadRun.cs` (`skills.list`/`skills.read`), `docs/skills-support-proposal.md` | High |
