...
| A3 | **Azure SQL** | Reuse Waterfall-proven config: Standard **S2** (50 DTU), 250 GB max, public access + Azure-services firewall rule, managed-identity connection string in Key Vault. Database name default **`guideants`**. | 1, 3 |
...
| A9 | **Scaling** | All container apps: **minReplicas=1**, consumption workload profile. No independent autoscale tuning in v1. | 2 |
...
- **FTS:** EF migrations conditionally enable full-text on `DocumentChunks`; Azure SQL
  Standard tier supports FTS (same class as Waterfall deployment).
...
| A3 | LOCKED | 2026-07-23 | Waterfall SQL SKU/pattern |
...
| A9 | LOCKED | 2026-07-23 | minReplicas=1 |
