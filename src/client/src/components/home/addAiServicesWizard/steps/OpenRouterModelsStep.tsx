import type { ExistingOpenRouterModel, OpenRouterModelDraft } from '../types';

interface OpenRouterModelsStepProps {
  existingModels: ExistingOpenRouterModel[];
...
export function OpenRouterModelsStep({
  existingModels,
  draftModels,
  draftModelId,
  ...
  onAddModel,
  onRemoveDraftModel,
}: OpenRouterModelsStepProps) {
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-gray-900">OpenRouter models (required)</h3>
        <p className="mt-1 text-sm text-gray-600">
          Add one or more OpenRouter chat model ids. This path uses provider <span className="font-mono">openrouter-chat</span>.
        </p>
      </div>

      <div className="grid grid-cols-1 gap-3 rounded border border-gray-200 bg-gray-50 p-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <div className="space-y-1">
          <label className="block text-xs font-semibold uppercase tracking-wide text-gray-600" htmlFor="wizard-openrouter-model-id">
            Model
          </label>
          <input
      ...
      <div className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-gray-600">Configured OpenRouter models</p>
        {existingModels.length === 0 && draftModels.length === 0 ? (
          <p className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
            Add at least one OpenRouter model to continue.
          </p>
        ) : (
          <div className="space-y-2">
