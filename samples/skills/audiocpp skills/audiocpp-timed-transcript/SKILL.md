...
description: "Industry-standard time-coded transcripts (SRT/WebVTT/JSON) via Max AudioCPP: product ASR for text + private qwen3_forced_aligner (/v1/tasks/run) for word times, collapsed to caption cues. Use when the user wants timed captions without PyTorch."
metadata:
  guideants:
    enabled: true
    display_order: 35
...
`qwen3_forced_aligner` over the Max raw gateway (`/private/v1/tasks/run`), then
...
ASR must already be loaded on Max (Settings / API lifecycle).

## Phase 0 gate (once per Max image)

```bash
python3 Output/Skills/audiocpp-timed-transcript/scripts/fetch_model.py \
...
# Max ROCm builds need --backend rocm (not cuda)
python3 Output/Skills/audiocpp-timed-transcript/scripts/spawn_engine.py start \
  --path /models-local/skill/asr/Qwen3-ForcedAligner-0.6B-GGUF \
  --family qwen3_forced_aligner --task align --backend rocm

python3 Output/Skills/audiocpp-timed-transcript/scripts/spawn_engine.py status
```

If status is not `ready` (e.g. missing model spec), update the Max `guideants-ai`
audiocpp image. Do not add a PyTorch aligner.

## Transcribe

```bash
python3 Output/Skills/audiocpp-timed-transcript/scripts/timed_transcribe.py \
  Output/uploads/clip.wav -o Output/clip --language English
```
...
- Max ROCm: `spawn_engine.py start ... --backend rocm`.
- Multi-model private engine (align+diar): `spawn_engine.py start ... --extra path=...;family=...;task=...`
  requires the updated skill gateway on Max (Gate M).
