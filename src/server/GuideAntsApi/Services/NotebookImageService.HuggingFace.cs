    ...
    public partial class NotebookImageService
    {
            ...
            string? requestPresetJson)
        {
            ...
            foreach (var route in routes)
            {
                ...
                {
                    lastError = $"{route.Provider}: {(int)response.StatusCode} {responseBody}";
            ...
            throw new InvalidOperationException(
                $"No working inference provider found for model '{textToImageModelId}' (text-to-image). Last error: {lastError}");
        }
            ...
            string? requestPresetJson)
        {
            ...
            throw new InvalidOperationException(
                $"No working inference provider found for model '{imageToImageModelId}' (image-to-image). Last error: {lastError}");
        }
