 ...
 = ({ src, width, height, poster }) => {
  ...
  const resolveUrl = (value: string): string => {
    ...
    if (isRelative && projectId && notebookId) {
      ...
      // Separate path from query string (e.g., "video.mp4?m=12345")
    ...
    if (isRelative && projectId && !notebookId && resolveProjectFilePath) {
      ...
      console.warn('[VideoNode] Could not resolve project file for relative path:', value);
    }
    return value;
  };
...
export class VideoNode extends DecoratorNode<React.JSX.Element> {
  ...
  static clone(node: VideoNode): VideoNode {
    return new VideoNode(node.__src, node.__width, node.__height, node.__poster, node.__key);
  }

  constructor(src: string, width?: 'inherit' | number, height?: 'inherit' | number, poster?: string, key?: NodeKey) {
    super(key);
    this.__src = src;
  ...
  static importJSON(serializedNode: SerializedVideoNode): VideoNode {
    const { src, width, height, poster } = serializedNode;
    const node = $createVideoNode({ src, width, height, poster });
    return node;
  }
  ...
  decorate(): React.JSX.Element {
    return (
      <div className="video-node my-4">
        <AuthenticatedVideo src={this.__src} width={this.__width} height={this.__height} poster={this.__poster} />
      </div>
    );
  }
}

export function $createVideoNode({ src, width, height, poster, key }: VideoPayload): VideoNode {
  return $applyNodeReplacement(new VideoNode(src, width, height, poster, key));
}
...
// Markdown transformer for video (uses token form [VIDEO:src] for import convenience)
export const VIDEO_TRANSFORMER: Transformer = {
  ...
  export: (node: LexicalNode) => {
    ...
    return `<video src="${node.getSrc()}" controls></video>`;
  },
  importRegExp: /\[VIDEO:([^\]]+)\]/,
  regExp: /\[VIDEO:([^\]]+)\]/,
