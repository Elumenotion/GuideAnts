...
description: "Experimental: use full raw audiocpp_server scenarios GuideAnts does not ship — speaker diarization, voice cloning, seeds, language forcing, voice-design, and deferred TTS families — via the Max token-gated raw gateway (/asr|/tts|/private) from a PC sandbox (or co-located engines if present). Use when the user asks for a voice, model, or audio scenario the built-in audio tools reject, including diarizing a meeting or labeling speakers."
metadata:
  guideants:
    enabled: true
    display_order: 30
...
**Default path: PC sandbox → Max raw audiocpp gateway.** That host is a
...
Optional: `HF_TOKEN` for gated downloads (runs on Max).
...
on Max, and spawn private engines there. See `references/engine-api.md`.
...
ASR / TTS must already be loaded on Max via GuideAnts Settings (API lifecycle).
Then:

```bash
# Synthesis with seed / language / clone
python3 Output/Skills/audiocpp-extended/scripts/engine_tool.py speech "Hello there" \
  -o Output/out.wav --voice-ref Output/uploads/user_voice.wav --seed 42

python3 Output/Skills/audiocpp-extended/scripts/engine_tool.py transcribe \
...
Downloads and private engines run on Max under `/models-local/skill/`. Pass the
usual dest paths; when the gateway is configured, scripts rewrite them:

```bash
python3 Output/Skills/audiocpp-extended/scripts/fetch_model.py <hf-repo> \
...
python3 Output/Skills/audiocpp-extended/scripts/spawn_engine.py status
python3 Output/Skills/audiocpp-extended/scripts/engine_tool.py speech "Hi" \
  --engine-url http://127.0.0.1:18099 --model <id> -o Output/hi.wav
...
If the sandbox runs **inside** the Max AI container and `AUDIOCPP_SKILL_BASE_URL`
...
- Use loaders not compiled into the Max binary (use **audiocpp-host-tts**).
