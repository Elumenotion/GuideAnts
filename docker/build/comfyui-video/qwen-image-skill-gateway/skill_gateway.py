...
transparent reverse proxy to the loopback adapter:
  ...
  /v1/image/jobs/{id}
  /v1/image/jobs/{id}/cancel
  /v1/image/jobs/{id}/result
  /v1/admin/models/install (optional)

Plus gateway-owned helpers:

  POST /files   stage an upload on Max (parity with audiocpp gateway)
  GET  /health  gateway + upstream probe

Auth: X-Qwen-Image-Skill-Token == GA_QWEN_IMAGE_SKILL_TOKEN.
"""
...
APP = FastAPI(title="GuideAnts qwen-image raw gateway", version="1.0.0")
...
def require_token() -> str:
    token = (os.getenv("GA_QWEN_IMAGE_SKILL_TOKEN") or "").strip()
    if not token:
        raise HTTPException(
            status_code=503,
            detail="GA_QWEN_IMAGE_SKILL_TOKEN is not configured on this host",
        )
    return token


def auth_dependency(
    x_qwen_image_skill_token: str | None = Header(default=None, alias="X-Qwen-Image-Skill-Token"),
) -> None:
    expected = require_token()
    if not x_qwen_image_skill_token or x_qwen_image_skill_token != expected:
        raise HTTPException(status_code=401, detail="invalid or missing X-Qwen-Image-Skill-Token")


def staging_root() -> Path:
    root = Path(os.getenv("GA_QWEN_IMAGE_SKILL_STAGING_DIR", "/var/lib/guideants/qwen-image-skill/staging"))
    root.mkdir(parents=True, exist_ok=True)
    return root


def adapter_url() -> str:
    host = os.getenv("GA_QWEN_IMAGE_ADAPTER_HOST", "127.0.0.1").strip() or "127.0.0.1"
    port = env_int("GA_QWEN_IMAGE_ADAPTER_PORT", 8190)
...
async def proxy_adapter(path: str, request: Request) -> Response:
    ...
    timeout = float(os.getenv("GA_QWEN_IMAGE_SKILL_PROXY_TIMEOUT_SECONDS", "3600") or "3600")
    try:
...
def health(_: None = Depends(auth_dependency)) -> dict[str, Any]:
    ...
    return {
        "status": "ok",
        "service": "qwen-image-raw-gateway",
        "api_version": "1",
        "ts": utc_now_iso(),
        "adapter": {
            "upstream": upstream,
        ...
        "note": (
            "Liveness only. Busy image jobs can stall adapter HTTP /health. "
...
async def proxy_v1(request: Request, path: str = "", _: None = Depends(auth_dependency)) -> Response:
...
def main() -> None:
    host = os.getenv("GA_QWEN_IMAGE_SKILL_HOST", "127.0.0.1").strip() or "127.0.0.1"
    port = env_int("GA_QWEN_IMAGE_SKILL_PORT", 8097)
    log_level = (os.getenv("GA_QWEN_IMAGE_SKILL_LOG_LEVEL") or "warning").strip() or "warning"
    staging_root()
